...
class Ebsd(object):
    ...
    def __makeEulerData(self):
        ...
        if "ang" in self._ebsdFormat:
            ...
            eulerData = np.array(
                [data["Euler1"], data["Euler2"], data["Euler2"]]).T
        else:
            raise ValueError(
                f"Unknown format {self._ebsdFormat} only ctf and ang are supported as of now")
    ...
    def applyMask(self, maskImge, maskSize=[30, 30], maskLocation=None, displayImage=False):
        ...
        if type(maskImge) == str:
            ...
            raise ValueError(
                "The supplied mask is neither a file path nor a valid boolen numpy array.")
...
# #         tmp2= np.arange(startInd[1],endInd[1])
        ...
        with open(pathName, "wb") as f:
            for line in self._header[:-1]:
                if isinstance(line, str):  # happens when header is string data
                    ...
                    # when header is read as binary byte data (from standard ang file)
                    f.write(line)
    ...
    def crop(self, start=(0, 0), dimensions=(10, 10)):
        ...
        """
        shape = self._shape
        cropParameters = start[0], start[0] + \
            dimensions[0], start[1], start[1]+dimensions[1],

        if start[0]+dimensions[0] > shape[0] or start[1]+dimensions[1] > shape[1]:
            raise ValueError(
                f"the ebsd data has shape {shape} but cropping parameters are :{cropParameters}")

        mask = np.zeros(shape=shape, dtype=bool)
        mask[cropParameters[0]:cropParameters[1],
             cropParameters[2]:cropParameters[3]] = True
        indx = np.where(mask.reshape(-1) == False)
        indx = indx[0].tolist()

        logging.info(
            f"removing {len(indx)} number of data points for croppiing the ebsd data")
        if "ang" in self._ebsdFormat:
            x, y = "x", "y"
        elif "ctf" in self._ebsdFormat:
            x, y = "X", "Y"
            ...
            raise ValueError(
                f"Uknown ebsd format {self._ebsdFormat} only '.ang' and '.ctf' are supported as of now")
        ...
        self._data[x] = self._data[x] - \
            self._data.iloc[ind, self._data.columns.get_loc(x)]
        ...
        for i, line in enumerate(self._header):
            if "# NCOLS_ODD:" in line:
                self._header[i] = self.__replace_numberFromHeader(
                    line, dimensions[1])
        ...
        self.__makeEulerData()
        self._autoName = self._autoName + \
            self.__replace_characters__(f"cropped_{dimensions}")
        logging.info(
            f"cropped the ebsd data points now the cropped size of the data is : {self._shape}")
