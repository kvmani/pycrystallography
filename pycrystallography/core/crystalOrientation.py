...
#         return self.__class__(self)
    ...
    def projectToFundamentalZone(self):
        ...
        if self._oriInfundamentalZone is None:
            ...
            eulerAnglesSet = [i.getEulerAngles(applyModulo=True).tolist() for i in symmetricOriSet]

            # Step 1: Filter the sets of Euler angles that are within the limits and keep track of their original indices
            filtered_euler_sets_with_indices = [(i, euler) for i, euler in enumerate(eulerAnglesSet) if all(np.array(euler) < limits)]
            
            # Step 2: If there are remaining valid sets, select the one based on the sum of angles and highest phi2 if tied
            if filtered_euler_sets_with_indices:
                if len(filtered_euler_sets_with_indices) == 1:
                    # If there is exactly one valid set, return it directly
                    euler = filtered_euler_sets_with_indices[0][1]
                    index = filtered_euler_sets_with_indices[0][0]
                else:
                    # If there are multiple valid sets, sort first by the sum of angles (max sum), 
                    # then by angle3 (phi2) in case of a tie
                    filtered_euler_sets_with_indices.sort(key=lambda x: (sum(x[1]), x[1][2]), reverse=True)  # Sort by sum and phi2
                    euler = filtered_euler_sets_with_indices[0][1]  # The Euler angles with the max sum (and highest phi2 if tied)
                    index = filtered_euler_sets_with_indices[0][0]  # Original index of the selected set
            else:
                # If no valid sets remain, revert to the original logic (this part stays the same as your original logic)
                warnings.warn("No valid Euler angles within the limits; reverting to original logic")
                euler = eulerAnglesSet[index]
                index = 0  # Assuming `index` should be initialized properly in this case

            # Step 3: Ensure the selected Euler angles are within the limits
            if all(np.array(euler) < limits):
                ...
                warnings.warn("Could not find the fundamental orientation based on the new logic. Trying old scheme.")
                assert all(np.array(euler) < limits), f"Could not find the fundamental Ori Something is Wrong !!!! {euler=} : {limits=}"
